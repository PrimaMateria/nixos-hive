#
# █▄ █ █ ▀▄▀ ▄▀▄ ▄▀▀
# █ ▀█ █ █ █ ▀▄▀ ▄██ homeServer
#
{
  inputs,
  cell,
}: let
  inherit (cell) bees machines installations system devices secrets;
in {
  bee = bees.boot;

  imports = [
    machines.beelink
    installations.common
    installations.headless
    system.networkingHome
    system.docker
    system.wireguard
    {
      networking.hostName = "homeServer";
      services.openssh.enable = true;
      networking.firewall.enable = false;

      primamateria.system.wireguard = {
        enable = true;
        endpoint = "primamateria.ddns.net";
        externalInterface = "enp2s0";
        privateKeyFile = secrets.wireguard.serverPrivateKeyFile;
        peers = [
          {
            name = "mentat";
            publicKey = secrets.wireguard.peers.mentat.publicKey;
            allowedIPs = ["10.100.0.2/32"];
            presharedKeyFile = secrets.wireguard.peers.mentat.presharedKeyFile;
          }
          {
            name = "stalker";
            publicKey = secrets.wireguard.peers.stalker.publicKey;
            allowedIPs = ["10.100.0.3/32"];
            presharedKeyFile = secrets.wireguard.peers.stalker.presharedKeyFile;
          }
        ];
      };

      users.users.primamateria.openssh.authorizedKeys.keys = [
        "ssh-rsa  primamateria@gg"
      ];
    }
  ];
}
